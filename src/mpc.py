...
# from parameters import T_pred, u_max, u_min, x_dtheta_max, x_dtheta_min, x_theta1_max, x_theta1_min, x_theta2_max, x_theta2_min, state_initial_perturbation, noise_sensor, noise_actuator
...
#         delta_x = x_mpc[:, tau] - x_ref
#         delta_u = u_mpc[:, tau] - u_ref
#         cost += cp.quad_form(delta_x, Q_tau) + cp.quad_form(delta_u, R_tau)
        
      
#     # Terminal cost
#     t_terminal = min(t_start + T_pred - 1, T-1)
#     delta_x_terminal = x_mpc[:, -1] - x_gen[:, t_terminal]
...
#             else:
#                 print(f"Warning: Problem not solved optimally with {solver}. Status: {problem.status}")
#         except cp.error.SolverError as e:
#             print(f"Solver error with {solver}: {e}")

#     # If no solver succeeds
#     print("Both ECOS and OSQP failed to solve the problem.")
...
#     # MPC loop
#     for t in range(T-1):
#         x_t = x_real_mpc[:,t] + noise_sensor*np.random.randn(x_size)
...
#         u_real_mpc[:,t] = u_mpc_t[:,0] + noise_actuator*np.random.randn(u_size)
