...
def J_Function(x_trajectory, u_trajectory, x_reference, u_reference, type):
    ...
    for t in range (T - 2):
        J = J + stage_cost(x_trajectory[:, t], 
                          x_reference[:, t], 
                          u_trajectory[:, t], 
                          u_reference[:, t], type,t)
    J = J + terminal_cost(x_trajectory[:, T-1], x_trajectory[:, T-1], type)
    
    return J

def stage_cost(x_stage, x_reference, u_stage, u_reference, type, t):
    ...
    match type:
        case "Affine":
            ...
            J_t =  qt.T @ delta_x\
                +  rt.T @ delta_u\
                + (1/2) * delta_x.T @ Qt[:,:,t] @delta_x \
                + (1/2) * delta_u.T @ Rt[:,:,t] @delta_u
        ...
        case "LQR":
            J_t = (1/2) * delta_x.T @ Qt[:,:,t] @delta_x \
                + (1/2) * delta_u.T @ Rt[:,:,t] @delta_u
...
def grad1_J(x_trajectory, x_reference, t):
    """
    ...
    """
    return Qt[:,:,t] @ (x_trajectory - x_reference)

def grad2_J(u_trajectory, u_reference,t):
    """
    ...
    """
    return Rt[:,:,t] @ (u_trajectory - u_reference)
